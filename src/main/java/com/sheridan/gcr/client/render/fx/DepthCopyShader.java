...
@OnlyIn(Dist.CLIENT)
public class DepthCopyShader  {
    public static int programId = -1;
    public static int depthSamplerLoc = -1;
    ...
    public static void init() {
        ...
        ResourceLocation vshPath = GCR.RL("shaders/core/depth_copy.vsh");
        ResourceLocation fshPath = GCR.RL("shaders/core/depth_copy.fsh");

        String vshSource = load(manager, vshPath);
        String fshSource = load(manager, fshPath);

        if (vshSource == null || fshSource == null) {
            System.out.println("Failed to load shader files DepthCopyShader.");
            return;
        }
        ...
        if (GL20.glGetShaderi(vsh, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            ...
            System.out.println("failed to compile [DepthCopyShader] VSH! \n" + log);
        ...
        if (GL20.glGetShaderi(fsh, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            ...
            System.out.println("failed to compile [DepthCopyShader] FSH! \n" + log);
        ...
        if (GL20.glGetProgrami(programId, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
            ...
            System.out.println("failed to link [DepthCopyShader] Program! \n" + log);
            programId = -1;
            isOK = false;
            return;
        }
        ...
        depthSamplerLoc = GL20.glGetUniformLocation(programId, "DepthSampler");
        ...
        System.out.println("DepthCopyShader init with: \n" + print());
    }

    public static String print() {
        return "DepthCopyShader{\n" +
                "programId=" + programId +
                ", \nDepthSampler=" + depthSamplerLoc +
                ", \nvaoId=" + vaoId +
                ", \nvboId=" + vboId +
                ", \nposLoc=" + posLoc +
                ", \nuvLoc=" + uvLoc +
