...
public class NumProp extends Prop {
    public float value;
    private float minRatio;
    private float maxRatio;
    private final Num ratio;
    public NumProp(String name, float value, float minRatio, float maxRatio, float defaultRatio) {
    ...
    public NumProp(String name, float value, float minRatio, float maxRatio) {
        this(name, value, minRatio, maxRatio, 1.0f);
    }

    public NumProp(String name, float value) {
        this(name, value, 0.01f, 1e8f, 1.0f);
    }
    ...
    public void loadFromJson(JsonObject jsonObject) {
        ...
        if (jsonElement != null && jsonElement.isJsonObject()) {
            ...
            value = prop.getAsJsonPrimitive("value").getAsFloat();
    ...
    public void loadData(CompoundTag tag) {
        if (tag.contains(ratio.name)) {
            ...
            if (tag1 instanceof CompoundTag compoundTag) {
                value = compoundTag.getFloat("value");
